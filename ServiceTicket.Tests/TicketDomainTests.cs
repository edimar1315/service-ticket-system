...
public class TicketDomainTests
{
    ...
    {
        var ticket = new Ticket("Cliente A", "Sem conexão", Priority.High);
...
public class TicketApplicationServiceTests
{
    ...
    public async Task UpdateTicketStatusAsync_WhenFinished_ShouldPublishEvent()
    {
        ...
        var ticket = new Ticket("Cliente B", "Impressora falhando", Priority.Medium);
    ...
    private sealed class InMemoryTicketRepository : ITicketRepository
    {
        ...
        public Task<IEnumerable<Ticket>> GetAllAsync(TicketStatus? status = null, Priority? priority = null, string? clientName = null, int pageNumber = 1, int pageSize = 10, CancellationToken cancellationToken = default)
            => Task.FromResult(_tickets.Values.AsEnumerable());

        public Task<int> GetTotalCountAsync(TicketStatus? status = null, Priority? priority = null, string? clientName = null, CancellationToken cancellationToken = default)
